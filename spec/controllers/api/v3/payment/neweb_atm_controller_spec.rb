...
describe Api::V3::Payment::NewebAtmController, :api_v3, type: :controller do
  context '#begin', signed_in: :normal do
    Given(:order) { create :pending_order, :with_public_work, user: user, payment_method: 'neweb/atm' }
    context 'returns order payment account with basic params provided', :vcr do
      ...
      And { response_json['payment_method'] == 'neweb/atm' }
      And { response_json['virtual_account'] == order.payment_id }
      And { response_json['bank_id'] }
    end
    ...
    context 'returns ok with free check order' do
      Given(:zero_price_order) { create :order, user: user, payment_method: 'neweb/atm' }
      When do
    ...
    context 'returns 400 with non-pending order' do
      Given(:paid_order) { create :paid_order, user: user, payment_method: 'neweb/atm' }
      When do
