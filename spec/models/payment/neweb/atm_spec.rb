...
describe Payment::Neweb::ATM do
  describe '#pay' do
    it 'sets payment_id to virtualaccount', :vcr do
      ...
      expect(order.payment).to eq('neweb/atm')
    ...
    it 'creates "pay" activity with payment info', :vcr do
      ...
      expect(activity.extra_info[:payment_method]).to eq('neweb/atm')
    end
  ...
  describe '#to_hash' do
    it 'returns payment method, bank id and virtual account', :vcr do
      ...
      expect(payment.to_hash).to eq(payment_method: 'neweb/atm',
                                    bank_id: '007',
                                    bank_name: '第一銀行',
                                    virtual_account: order.payment_id)
    end
  end
end
