...
RSpec.describe 'api/v3/works/index.json.jbuilder', type: :view do
  let!(:work) { create(:work) }
  let(:works) { Work.page(1) }
  let(:request_query) { { 'query' => 'blah' } }

  it 'renders layer', :freeze_time do
    assign(:works, works)
    assign(:request_query, request_query)
    render
    expect(JSON.parse(rendered)).to eq(
      ...
      'meta' => {
        ...
        'current_page_count' => works.count,
        'current_page'       => works.current_page,
        'per_page'           => works.per_page,
        'total_count'        => works.total_entries,
