...
RSpec.describe 'api/v3/carts/show.json.jbuilder', :caching, type: :view do
  let(:order) do
    o = create(:order, :priced)
    Api::V3::OrderDecorator.new(o)
  end
  let(:current_currency_code) { 'TWD' }

  it 'renders cart' do
    order.stub(:pricing_identifier).and_return('bf53b9c3fbb4434af85e8a3c06b70abb')
    assign(:order, order)
    assign(:cart, double(items: order.order_items))
    assign(:current_currency_code, current_currency_code)
    render
    expect(JSON.parse(rendered)).to eq(
      'cart' => {
        ...
        'display_price' => {
          ...
          'shipping_fee' => render_price(order.shipping_price, currency_code: order.currency),
          'price' => view.render_price(order.price, currency_code: order.currency),
          'price_without_shipping_fee' => view.render_price(order.price_without_shipping_fee, currency_code: order.currency),
        },
