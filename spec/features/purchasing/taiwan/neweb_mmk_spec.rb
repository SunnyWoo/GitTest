...
feature 'Payment/Taiwan/Neweb MMK', :js, come_from: 'Taiwan' do
  before do
    # 建立新台幣
  ...
  scenario '可看到正確的總結內容' do
    ...
    choose :order_payment_neweb_mmk
    click_link '結帳明細'
    expect(page).to have_content('ayaya')
    expect(page).to have_content('超商繳費')
  end

  scenario '可以使用超商繳費' do
    visit '/'
    click_link 'My great work'
    click_link '加入購物車'
    click_link '立刻結帳'
    ...
    choose :order_payment_neweb_mmk
    click_link '結帳明細'
    expect(page).to have_link('Pay with 超商繳費')
  end
end
