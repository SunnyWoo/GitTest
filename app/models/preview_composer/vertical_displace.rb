...
class PreviewComposer::VerticalDisplace < PreviewComposer
  store :layers, coder: JSON

  def self.available_params
    %w(background_width background_height background_file case_file
       mask_file mask_left mask_top mask_width mask_height mask_displace)
  end
  ...
  concerning :MaskConcern do
    included do
      require_relative 'vertical_displace/mask'
  ...
  concerning :BuildOrderImage do
    ...
    def build_order_image(cover)
      ...
      # 遮罩
      Rails.logger.debug 'Loading case mask image...'
      ...
      # 縮小作品圖
      Rails.logger.debug 'Resizing print image...'
      cover.resize size_from_dimension(mask.dimension)
      # 貼上作品圖
      Rails.logger.debug 'Drawing print image...'
      canvas = canvas.composite(cover) do |c|
        c.geometry position_from_dimension(mask.dimension)
      end
      # 位移作品圖
      Rails.logger.debug 'Displace image...'
      canvas = canvas.composite(case_mask) do |c|
        c.displace "0x#{mask.displace}"
      end
      # 畫上殼
