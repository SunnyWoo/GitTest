...
@CPA.Assets.Search = React.createClass
  propTypes:
  ...
  render: ->
    filterInputs = @state.filters.map (item) =>
      if item.type isnt 'search'
        <CPA.Assets.FilterInput key={item.id}
                                filterID={item.id}
                                type={item.type}
                                value={item.value}
    ...
    <div className='coupon-filters'>
      <div>
        <CPA.Base.DefaultFilter name="Name" />
        {filterInputs}
      ...
      <div className="coupon-search clearfix">
        <CPA.Base.TextInput className="form-control pull-left"
                            ...
                            placeholder="Startin typing code name or coupon code to search."
